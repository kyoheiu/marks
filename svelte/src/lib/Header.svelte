...
<div
  class="sticky top-0 flex w-full flex-col items-center bg-item_background text-sm shadow"
>
  <div class="flex h-12 w-64 items-baseline pt-2 sm:w-120 md:w-144">
    ...
    {:else}
      <button
        on:click={addItem}
        class="rounded border-2 border-base_border bg-item_background px-2 py-1 text-xs font-semibold text-base_color"
        >+New</button
      >
    {/if}
    &nbsp; &nbsp;
